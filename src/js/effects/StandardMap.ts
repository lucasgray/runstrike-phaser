...
export default class StandardMap {
    ...
    static AddMapEffects(game: Phaser.Game) : Phaser.Group {
        ...
        this.addBlueShader(game);

        return grouped;
    }

    static addBlueShader(game: Phaser.Game) {
        ...
        // load the image
        var lut = game.add.sprite(game.world.centerX, game.world.centerY, 'lut');
        lut.exists = false;

        // structure uniforms
        var customUniforms = {
            iChannel0: { type: 'sampler2D', value: lut.texture, textureData: { repeat: true } }
        };

        // apply filter
        var filter = new Phaser.Filter(game, customUniforms, fragmentSrc);
        filter.setResolution(640, 960);
